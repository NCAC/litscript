...
export async function run() {
  ...
  if (!configPath) throw Error(`Could not find '${cfg.tsconfig}'.`);
  ...
  let cl = ts.parseJsonConfigFileContent(
    configFile.config,
    parseConfigHost,
  ...
  if (opts.watch) {
    ...
    for (let diag of ts.getPreEmitDiagnostics(prg)) log.reportDiagnostic(diag);
