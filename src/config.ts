...
export function getTemplate(): tmp.Template {
  if (!template) {
    ...
    else options.frontMatter = template.frontMatterDefaults();
  }
  return template;
}
/**
    ...
    fs.watch(tempdir, { recursive: true }, (_, filename) => {
      ...
      if (require.cache[module]) {
        for (const mod in require.cache)
          if (mod.startsWith(tempdir)) delete require.cache[mod];
        log.info(
          `Template module ${log.Colors.Blue}${filename}${log.Colors.Reset} changed. Clearing cache.`
        );
        template = undefined;
      }
    });
