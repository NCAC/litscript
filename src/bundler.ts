...
function findNodeModulesDir(): string {
  ...
  if (path.basename(res) == nmdir) return res;
...
function initConfig(codeFiles: CodeFiles) {
   ...
   */
  config.mode = opts.deployMode == "dev" ? "development" : "production";
  if (opts.deployMode == "dev") config.devtool = "inline-source-map";
  /**
...
export function bundle(codeFiles: CodeFiles) {
  ...
  if (cfg.getOptions().watch) compiler.watch({}, handler);
...
export async function finished() {
  while (!done) await new Promise((resolve) => setTimeout(resolve, 1000));
}
